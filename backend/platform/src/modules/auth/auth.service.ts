...
export class AuthService {
  ...
  async register(email: string, password: string) {
    ...
    if (existing) throw new AppError(409, ErrorCode.EMAIL_ALREADY_EXISTS, 'Email already in use')
  ...
  private signToken(userId: string): string {
    return jwt.sign({ sub: userId }, env.JWT_ACCESS_SECRET, { expiresIn: env.JWT_ACCESS_EXPIRES_IN as jwt.SignOptions['expiresIn'] })
  }
}
