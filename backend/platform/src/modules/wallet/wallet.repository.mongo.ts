...
export class MongoWalletRepository implements IWalletRepository, OnModuleInit {
  async onModuleInit() {
    await getDb().collection('wallets').createIndex({ userId: 1 }, { unique: true })
  }

  async findById(id: string): Promise<Wallet | null> {
    const doc = await getDb().collection('wallets').findOne({ _id: new ObjectId(id) })
    return doc ? this.toWallet(doc) : null
  }

  async findByUserId(userId: string): Promise<Wallet | null> {
    const doc = await getDb().collection('wallets').findOne({ userId: new ObjectId(userId) })
    return doc ? this.toWallet(doc) : null
  }

  async create(userId: string, balance: number): Promise<Wallet> {
    const now = new Date()
    const result = await getDb().collection('wallets').insertOne({ userId: new ObjectId(userId), balance, createdAt: now, updatedAt: now })
    return { id: result.insertedId.toHexString(), userId, balance, createdAt: now, updatedAt: now }
  }
  ...
  async deductBalance(userId: string, amount: number): Promise<Wallet | null> {
    const doc = await getDb().collection('wallets').findOneAndUpdate(
