...
import { WALLET_REPOSITORY, IWalletRepository } from './wallet.repository.interface'
...
export class WalletService {
  ...
  async getBalance(userId: string): Promise<number> {
    ...
    if (!wallet) throw new AppError(404, ErrorCode.NOT_FOUND, 'Wallet not found')
  ...
  async deductBalance(userId: string, amount: number): Promise<Wallet> {
    ...
    if (!wallet) throw new AppError(400, ErrorCode.INSUFFICIENT_BALANCE, 'Insufficient balance')
    return wallet
  }
