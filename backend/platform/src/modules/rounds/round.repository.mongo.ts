...
export class MongoRoundRepository implements IRoundRepository, OnModuleInit {
  ...
  async findById(id: string): Promise<Round | null> {
    const doc = await getDb().collection('rounds').findOne({ _id: new ObjectId(id) })
  ...
  async create(crashPoint: number): Promise<Round> {
    ...
    return { id: result.insertedId.toHexString(), phase: 'WAITING', crashPoint, startedAt: null, crashedAt: null, createdAt: now }
  }

  async updatePhase(id: string, phase: GamePhase, extra: Partial<Round> = {}): Promise<Round> {
    const { id: _id, ...rest } = extra
    void _id
    const doc = await getDb().collection('rounds').findOneAndUpdate(
      { _id: new ObjectId(id) },
      { $set: { phase, ...rest } },
  ...
  async findRecent(limit: number): Promise<Round[]> {
    const docs = await getDb().collection('rounds').find({ phase: 'CRASHED' }).sort({ createdAt: -1 }).limit(limit).toArray()
