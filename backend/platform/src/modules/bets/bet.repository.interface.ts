...
export interface IBetRepository extends IBaseRepository<Bet> {
  ...
  findBySlot(roundId: string, userId: string, slotId: BetSlotId): Promise<Bet | null>
  findActiveByRound(roundId: string): Promise<Bet[]>
  findActiveByUser(userId: string, roundId: string): Promise<Bet[]>
  findByUser(userId: string, limit: number, cursor?: string): Promise<{ bets: Bet[]; nextCursor: string | null }>
  cashOut(betId: string, multiplier: number, payout: number): Promise<Bet | null>
